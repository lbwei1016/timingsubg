...
void print_qdMap(qdMap *map)
{
	cout << "printing qdMap [Format: (query edge id, data edge id)]\n";
	cout << "----------------------\n";
...
bool query::compatible(qEdge *_qe1, dEdge *_e1, qEdge *_qe2, dEdge *_e2)
{
	...
	{
		stringstream _ss;
		_ss << "*** (query edge" << _qe1->id << " -> "
			<< "query edge" << _qe2->id << ") is compatible with"
			<< "(data edge" << _e1->id << " -> "
			<< "date edge" << _e2->id << ")\n";
		util::track(_ss);
	}
