...
public:
	...
	bool get_mslist(list<shared_ptr<msNode>> &_mslist);
	...
	shared_ptr<msNode> remove_edge(dEdge *_e, lockReq *_lr);
	shared_ptr<msNode> remove_match(set<match *> &_mset);
	//	shared_ptr<msNode> remove_match(List<match>* _mslist);
	/* remove _ms and return _ms->next */
	shared_ptr<msNode> remove_ms(shared_ptr<msNode> _cur_ms);
	bool add_rmlist(shared_ptr<msNode> &_ret_mslist, shared_ptr<msNode> &_tail, shared_ptr<msNode> _cur_ms);

	/* head_mslist contains no data, initialed as new msNode */
	bool add_msnodes(list<shared_ptr<msNode>> *_mlist);
	bool add_msnodes(shared_ptr<msNode> _mlist);

	void get_all_matches(List<match> *_mlist, bool _is_level_mat = false);

	static void build_matlist(List<match> &_matlist, shared_ptr<msNode> _mlist_head);
