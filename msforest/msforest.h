...
private:
	 ...
	 * */
	// bool insert(teNode* _node, lockReq* _lr, List<JoinResult>* _msN2matches, list<shared_ptr<msNode>> * _branches = NULL);
	bool insert(teNode *_node, nodeOP *_lr, List<JoinResult> *_msN2matches, list<shared_ptr<msNode>> *_branches = NULL);
	string new_match_str(list<shared_ptr<msNode>> *_branches);
	string expired_match_str(shared_ptr<msNode> _mslist);
	static string matches_str(List<match> *_matlist);
	static string matches_str(shared_ptr<msNode> _mslist);
	/* _node is at_left()
	 ...
	 * */
	bool join_right(list<shared_ptr<msNode>> *_branch_nodes, teNode *_node, nodeOP *_lr, List<JoinResult> &_jrlist);
	// bool join_right(list<shared_ptr<msNode>> * _branch_nodes, teNode* _node, lockReq* _lr, List<JoinResult>& _jrlist);
	/*
	 ...
	 * */
	bool further_join(teNode *_cur_te, list<shared_ptr<msNode>> *_branches, OPlist *_lrlist);
	// bool further_join(teNode* _cur_te, list<shared_ptr<msNode>> * _branches, LRlist* _lrlist);

	/* return removed_list
	 * if(_rm_fathers==NULL) return NULL;
	 * _node could be upper or lower
	 * */
	shared_ptr<msNode> remove(teNode *_node, shared_ptr<msNode> _rm_fathers, lockReq *_lr);
	/* only called for upper remove */
	shared_ptr<msNode> remove(teNode *_node, List<match> *_matches, lockReq *_lr);
	void gc_release(vector<shared_ptr<msNode>> &gclist_vec);
